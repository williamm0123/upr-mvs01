...
## A.9 W0-B 尾部统计判决（2026-08-24 实测）

W0 跑满 30k（29990 步 / 18.6 小时），`val abs_err 2.6267 (best@17k) / 2.6676 (@30k)`、`acc@2mm 0.9003`、`acc@8mm 0.9607`。

**val 曲线 17k 之后进入平台**：12k 2.758 → 18k 2.703 → 21k 2.697 → 24k 2.697 → 27k 2.638 → 30k 2.668。best 落在 17k 只是平台上的幸运点。v3 那句「跑到 30k 必然还有明显提升不能预设」，实测答案是**后 13k 步几乎没买到东西**，振荡带宽约 ±0.03 mm。这给了 A.6.3 一个噪声下界：**W1 的改善要超过约 0.05 mm 才算数**。

### W2：放弃

在 W0 best 上跑 `scripts/tail_posterior_stats.py`（val 833 样本，0.8 整幅、5 视角）：

| 预注册判据 | 实测 | 阈值 |
|---|---|---|
| `P(dep 峰存在 ∧ M2≥0.10 ∧ GT 在邻域 \| A∪B)` | **0.0133** | ≥0.35 做 / <0.20 放弃 |
| oracle 上界 `P_orc_gt_in` | 0.0341 | — |
| `mass_dep_p50` | 0.0001 | — |
| `P_winner_only_cov` → `P_top2_cov_dep` | 0.9235 → 0.9479 | 只 +2.4 pp |

差放弃线还有一个数量级。更关键的是 **oracle 上界只有 0.034** —— 完美的模态选择器也只能捞回尾部的 3.4%。尾部不是「选错表面」，是「窗口大体在对的地方但没框住」。**不为 W2 扩模型。**

### 重新定位：剩下的空间全在 stage4 覆盖率

```
stage4 in_range = 0.888     窗内 abs_err = 0.34–0.66 mm
                            窗外 abs_err = 15.1–17.6 mm
```

**窗内精度已经达标**（工单的 0.5 mm 目标在那 89% 像素上已经做到）。当前 2.63 mm 几乎全部来自没框住的 11%：`0.888×0.45 + 0.112×16.5 ≈ 2.2 mm`，与实测吻合。

按深度分桶，远端最差：

| | q0 | q1 | q2 | q3(远端) |
|---|---|---|---|---|
| stage4 in_range | 0.871 | 0.889 | 0.840 | **0.672** |
| abs_err (mm) | 2.46 | 2.26 | 2.80 | **5.87** |
| 像素占比 | 0.100 | 0.517 | 0.324 | 0.059 |

覆盖率 0.888 → 0.95 而窗内精度不变，val abs_err 就能到约 1.2 mm。这正是 W1 的靶心：pinball 的 τ₄=0.92 直接把 stage4 覆盖率当优化目标，逆深度轴针对的正是 q3 的远端分辨率。

### rank 分布：信息在 stage1 就有，丢在交接上

v3 预注册要看「rank(g) 分布：尾部 vs 全体」，实测**不一致**：
...
读法：尾部里 GT 所在的 bin **有 75% 仍落在 stage1 48-bin 后验的 top-3**，只是不在 top-1。也就是说信息在 stage1 就已经有了，丢失发生在「只把 argmax 传给下一级、以它为中心开窗」这个**交接**上。这与 W2 判决一致 —— 需要的不是再加一个模态，而是范围中心/交接更稳。

（另注：`P_dep_found = P_orc_found = 1.0000` 不能读成「总有第二个表面」。48 bin 的轴上，任何离 winner 超过 `2·h_sep` 的局部极大都算「找到」，这个判据近乎恒真。有没有东西要看 `M2`，不看 `found`。）

**因此 W1 的判读优先级重排为**：
1. `stage4/in_range` > 0.888，尤其 `val/q3_s4_in_range` > 0.672
2. `val/q3_abs_err` < 5.87
3. `interval_mm_p50` 逐级下降、`gap_ratio_final_p99 ≤ 1.001`（A.6 的结构条件）

**一个不成立的比较**：首跑 W1 的 `stage4/in_range = 0.838` 不能拿来和 W0 的 0.888 比 —— 那是 2040 步的未收敛模型。要等 W1 跑满 30k。
