...
# arm R_vis
#
# R + 逐 (source, pixel) 可见性加权。上一轮它近似恒等映射 (归一化熵 0.974,
# 有效源视角 3.865/4, 最大权重 0.316 对均匀值 0.25, 集中像素 0.03%),
# 预期效应可能低于噪声地板 —— 这个 arm 是来确认它确实没有贡献的。
#
#     sbatch scripts/arm_R_vis.sh
#
# 单卡 A100, 12000 步, 约 3 小时。结果写 log/experiments/R_vis/。
