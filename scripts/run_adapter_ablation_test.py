#!/usr/bin/env python
"""Run fixed-module DINO adapter ablations before cost-volume construction."""
...
from experiments.runners import run_adapter_ablation_test
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("outputs/adapter_ablation"))
    ...
    parser.add_argument("--single-layer", type=int, default=6)
    parser.add_argument("--fusion-layers", default="1,6,12")
...
def main() -> None:
    args = parse_args()
    fusion_layers = parse_layer_ids(args.fusion_layers)
    extracted_layers = tuple(sorted({args.single_layer, *fusion_layers}))
    ...
    dino_config = DINOConfig(
        ...
        layers=tuple(layer_id - 1 for layer_id in extracted_layers),
    )
    adapter_config = AdapterConfig(
        single_layer_id=args.single_layer,
        layer_ids=fusion_layers,
    ...
    result = run_adapter_ablation_test(
        ...
        device=args.device,
        export_pointclouds=not args.no_pointclouds,
    )
