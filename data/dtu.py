...
class DTUMVSDataset(Dataset):
    def __init__(
        ...
        **kwargs):
        ...
        # 无 depth GT。测试按 DTU 官方协议评测: 融合点云与 STL 参考点云比对
        # (accuracy/completeness/overall), 不使用 depth GT。
    ...
    def precrop_inputs(self, idx):
        ...
        for i, view_id in enumerate(view_ids):
            if self.is_test:
                # dtu_testing/<scan>/images/000000xx.jpg, cams/000000xx_cam.txt (无 depth GT)
            ...
            if i == 0:
                # 测试集无 depth GT; depth_values 仍从相机参数的 depth_min/interval 生成。
                if not self.is_test:
